...
insert into KHACHHANG
	values
		...
		('khso000003',N'Công ty Việt Tiến',N'giao dịch tiền mặt',N'Hồ Chí Minh','khangheheqt3@gmail.com','0112233442','kh12345672'),
...
VALUES
    ('CTso000001', N'Việt Tiến', N'giao dịch bitcoin', N'Địa chỉ A', '0123456789', '0987654321', 'emailA@gmail.com'),
    ('CTso000002', N'Công ty B', N'giao dịch tiền ảo', N'Địa chỉ B', '0123756789', '0987654322', 'emailB@gmail.com'),
...
INSERT INTO MATHANG
VALUES 
	('MH00000001', N'Tivi Sony', 'CTso000001', 'LHso000001', 10, N'Máy', 5000000),
	...
	('MH00000007', N'Sữa hộp XYZ', 'CTso000002', 'LHso000005', 107, N'Hũ', 1600000),
...
VALUES 
    ...
    ('SHD0000003', 'MH00000005', 18000000, 190, 2), 
    ('SHD0000004', 'MH00000006', 6000000, 120, 3),  
    ('SHD0000005', 'MH00000007', 1600000, 90, 2), 
...
SELECT  HO + ' '+ TEN AS HoTen,  DIACHI AS DiaChi,  YEAR(NGAYLAMVIEC) AS NamBatDauLam
FROM NHANVIEN;
...
WHERE TENGIAODICH IN (SELECT TENGIAODICH FROM NHACUNGCAP)
  AND TENCONGTY IN (SELECT TENCONGTY FROM NHACUNGCAP);
...
SELECT MANHANVIEN, 
    (SELECT COUNT(*) FROM DONDATHANG WHERE DONDATHANG.MANHANVIEN = NHANVIEN.MANHANVIEN) AS SoLuongDonHang
FROM NHANVIEN;
...
SELECT 
    MACONGTY AS 'Mã công ty',
    TENCONGTY AS 'Tên công ty',
    TENGIAODICH AS 'Tên giao dịch',
    DIACHI AS 'Địa chỉ',
...
FROM NHACUNGCAP
WHERE TENGIAODICH = 'Công ty VINAMILK';
...
SELECT 
    ...
    NCC.TENCONGTY AS 'Nhà cung cấp' 
FROM MATHANG MH
JOIN NHACUNGCAP NCC ON MH.NHACUNGCAPNO = NCC.MACONGTY;
--8.Loại hàng thực phẩm do những công ty nào cung cấp và địa chỉ của các công ty đó là gì?
SELECT DISTINCT 
    NCC.TENCONGTY AS 'Tên công ty cung cấp', 
    NCC.DIACHI AS 'Địa chỉ công ty' 
FROM MATHANG MH
JOIN NHACUNGCAP NCC ON MH.NHACUNGCAPNO = NCC.MACONGTY
WHERE MH.TENHANG = 'Thực phẩm';
--9.Những khách hàng nào (tên giao dịch) đã đặt mua mặt hàng Sữa hộp XYZ của công ty?
SELECT DISTINCT 
    KH.TENCONGTY AS 'Tên giao dịch khách hàng' 
FROM KHACHHANG KH
JOIN DONDATHANG DH ON KH.MAKHACHHANG = DH.KHACHHANGNO
JOIN CHITIETDATHANG CTDH ON DH.SOHOADON = CTDH.DONDATHANGNO
JOIN MATHANG MH ON CTDH.MATHANGNO = MH.MAHANG
...
SELECT 
    KH.TENCONGTY AS 'Khách hàng đặt hàng',
    NV.TEN AS 'Nhân viên lập đơn',
    DH.NGAYDATHANG AS 'Thời gian lập đơn',
    DH.NOIGIAOHANG AS 'Địa điểm giao hàng'
FROM DONDATHANG DH
JOIN KHACHHANG KH ON DH.KHACHHANGNO = KH.MAKHACHHANG
JOIN NHANVIEN NV ON DH.MANHANVIEN = NV.MANHANVIEN
WHERE DH.SOHOADON = 'SHD0000001';
...
SELECT 
    ...
    (LUONGCOBAN + PHUCAP) AS 'Tổng lương'
FROM NHANVIEN;
...
SELECT 
    ...
    STUFF((SELECT ', ' + TEN
           FROM NHANVIEN AS NV
           WHERE NV.NGAYSINH = N.NGAYSINH
           FOR XML PATH('')), 1, 2, '') AS 'Danh sách nhân viên'
FROM NHANVIEN AS N
GROUP BY NGAYSINH
HAVING COUNT(*) > 1;
